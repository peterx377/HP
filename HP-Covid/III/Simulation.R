...
Simulation <- function(covid, reproNum, param) {
  ...
  T <- ncol(newcovid)
  newdays <- T - ncol(oldcovid)
  newbins <- newdays%/%10 # calculate new bins needed
  
  # retrieve params
  reproNum_ <- read.csv(file = './data/II/param/R0_state.csv')
  param_ <- read.csv(file = "./data/II/param/weibull_param.csv")
  # duplicate bins at the end 
  if (newbins>0) {
    for (c in 1:newbins) {
      reproNum_ <- cbind(reproNum_, reproNum_[,ncol(reproNum_)])
    }    
  }
  # State 
  ...
  for (state in 1:2) {
    ...
    reproNum <- as.numeric(c(reproNum_[1,]))
