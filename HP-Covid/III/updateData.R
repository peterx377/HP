...
library(sys)
setwd("~/PP/covid/3.0")
# Update the true daily covid case count
updateData <- function() {
  # retrieve case count in history and format its dates
  caseHist <- read.csv(file = './data/II/state_covid_confirmed.csv')
  ...
  dataEnd <- as.Date("11.01.2021", "%m.%d.%Y")
  ...
  # data retrieval 
  # Alert: there could be a bug every time entering a new year
  i = 1
  ...
  write.csv(newData,"./data/III/state_covid_confirmed.csv",row.names=F)
}
